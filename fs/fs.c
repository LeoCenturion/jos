...
alloc_block(void)
{
	...
	for (uint32_t blockno = 0; blockno < super->s_nblocks; blockno++) {
		if (block_is_free(blockno)) {
			bitmap[blockno / 32] &= ~(1 << (blockno % 32));
			flush_block((void*)(blockno*BLKSIZE+DISKMAP+2*BLKSIZE));
			return blockno;
		}
	}
	return -E_NO_DISK;
...
file_block_walk(struct File *f, uint32_t filebno, uint32_t **ppdiskbno, bool alloc)
{
	...
	if(filebno >= (NDIRECT + NINDIRECT)){
		...
		*ppdiskbno = &f->f_direct[filebno];
		...
		vaddr = diskaddr(f->f_indirect);
		memset(vaddr, sys_getenvid(), BLKSIZE);
		flush_block(vaddr);
	}
	else{
	vaddr = diskaddr(f->f_indirect);
	...
	*ppdiskbno = &vaddr[filebno-NDIRECT];
	return 0;
}
...
file_get_block(struct File *f, uint32_t filebno, char **blk)
{
	...
	if (*ppdiskbno==0) {
			...
			*ppdiskbno = r;
			memset( diskaddr(*ppdiskbno), sys_getenvid(), BLKSIZE);
